#ifndef PSAIIM_MPI_ONLY_H
#define PSAIIM_MPI_ONLY_H
...
std::unordered_map<NodeId, double> MPIOnlyPR(const Graph& graph, const std::vector<std::vector<CompId>>& levels, const std::unordered_map<NodeId, CompId>& partition);
