#ifndef PSAIIM_SERIAL_H
#define PSAIIM_SERIAL_H
...
std::unordered_map<NodeId, double> SerialPR(const Graph& graph, const std::vector<std::vector<CompId>>& levels, const std::unordered_map<NodeId, CompId>& partition);
